...
$(document).on("click", ".btn_option_add", function () {
    ...
    addBox.append("<div class=\"input-group\"  style=\"width: 600px;\" ><select name='option_required' class='required form-select' >" +
        "<option value='Y'>필수옵션</option><option value='N'>선택옵션</option></select>" +
        "<select name='option_multiple' class='op_multi form-select ' ><option value='Y'>중복가능</option>" +
        "<option value='N'>중복불가능</option></select><input type='text' placeholder='옵션그룹명을 입력하세요' name='option_group'class='op_group form-control' style=\"width: 300px;\" maxlength=\"60\">" +
        "</div><div style=\"height: 5px\"></div><div class='group'><div class='optionName_div input-group' style='width: 600px'>" +
        "<input type='text'placeholder='옵션명을 입력하세요'name='option_name'class='op_name form-control' style=\"width: 300px;\" maxlength=\"60\">" +
        "<input style='width: 200px;' type='text' placeholder='옵션가격' name='option_price'class='op_price form-control' oninput=validNum() maxlength=\"15\">" +
        "<div style='position: relative; top: 5px; left: -25px;'>원</div><button type='button' class='btn_opnameadd deli_btn'>+</button></div></div>")
    //addBox.append(" <select name='option_required' class='required'> <option value='Y'>필수옵션</option> <option value='N'>선택옵션</option></select><select name='option_multiple' class='op_multi'>  <option value='Y'>중복가능</option><option value='N'>중복불가능</option></select>  <input type='text' placeholder='옵션그룹명을 입력하세요' name='option_group' class='op_group'><br><div class='group'> <div class='optionName_div'><input style='width: 200px' type='text' placeholder='옵션명을 입력하세요' name='option_name' class='op_name'>  <input style='width: 150px;'  type='text' placeholder='옵션가격' name='option_price' class='op_price' oninput=validNum()>원 &nbsp;&nbsp;<button type='button' class='btn_opnameadd deli_btn'>+</button></div> <br></div>")
...
$(document).on("click", ".btn_opnameadd", function () {
    ...
    opAddBox.append("<input type='text' placeholder='옵션명을 입력하세요' name='option_name' class='op_name form-control' style=\"width: 300px;\"maxlength=\"60\"> " +
        "<input type='text' placeholder='옵션가격' name='option_price' class='op_price form-control' style=\"width: 200px;\" oninput=validNum() maxlength=\"15\">" +
        "<div style='position: relative; top: 5px; left: -25px;'>원</div></div>")
        ...
        if(opGroup.length!=0){
            ...
            for(let i=0; i<opGroup.length; i++ ) { //중첩 for문 사용
                ...
                for (let j = 0; j < opName.length; j++) {
                    ...
                    $.ajax({
                        url: "/owner/menu/add/menuOptionAdd",
                        type: "post",
                        data: data
                    })
                }
